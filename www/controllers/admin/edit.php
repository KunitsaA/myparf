...
if(!empty($_FILES['userfile']['name'])){// Работаем с изображением
			...
			foreach($img_arr as $key => $val){ ++$i;		
				...
				if(move_uploaded_file($_FILES['userfile']['tmp_name'][$key], HOME_DIR.IMG_DIR.$val['name'])){ 
					...
					if($rr_img[0] > IMG_MAX_W){
						...
						$img_max = reSize($dir_upload_img,IMG_MAX_W,'max_','',IMG_DIR); 
						$del = @unlink($dir_upload_img);
		 
					} else { $img_max = $val['name']; }		
					
					mysql_query("UPDATE ".$type." SET img".$i."='".IMG_DIR."".$img_max."', ims".$i."='".IMG_DIR."".$img_min."', imr".$i."='".IMG_DIR."".$img_round."' WHERE id='".$arr['id']."'");
